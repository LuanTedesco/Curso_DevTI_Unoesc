...
function criarTabela(funcionarios) {
  ...
  if (funcionarios) {
    const linhas = funcionarios.map(funcionario => {
      ...
      const tdNumeroDependentes = document.createElement('td')
      tdNumeroDependentes.innerHTML = funcionario.numeroDependentes
      ...
      acaoAlterar.setAttribute(
        'href',
        FRONT + 'alterar_funcionario.html?id=' + funcionario.id
      )
      ...
      acaoExcluir.addEventListener(
        ...
        function (event) {
          if (confirm('Tem certeza que deseja excluir?')) {
            axios.delete(API + funcionario.id, {}).then(res => {
      ...
      tr.appendChild(tdNumeroDependentes)
